...
function EquipmentPicker({ onPick, onClose }) {
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    ...
    supabase
      .from('equipment')
      .select('id, sku, description, model, list_price, vendor_id, vendors:vendor_id(name)')
      .eq('active', true)
      .order('description')
      .limit(500)
      .then(({ data }) => {
  ...
  const filtered = useMemo(() => {
    if (!search.trim()) return items.slice(0, 100);
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-0 md:inset-x-auto md:right-8 top-8 md:top-16 bottom-8 md:bottom-16
                      md:w-[36rem] bg-white rounded-2xl md:rounded-lg shadow-elevated z-[70] overflow-hidden flex flex-col">
        <div className="px-4 py-3 border-b border-page-200 flex items-center justify-between bg-navy-900 text-chalk-50">
          <h3 className="text-sm font-medium">Pick equipment</h3>
        ...
        <div className="p-3 border-b border-page-200">
          <input type="search" value={search} onChange={(e) => setSearch(e.target.value)}
                 placeholder="Search description, SKU, model…" autoFocus
              ...
              <div className="flex items-baseline justify-between gap-2">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-0.5 text-[10px] text-slate-500">
                    ...
                    {it.vendors?.name && <span>· {it.vendors.name}</span>}
                    {it.model && <span>· {it.model}</span>}
                  </div>
                </div>
                {it.list_price != null && (
                  <div className="text-xs font-mono tabular-nums text-slate-700 flex-shrink-0">
